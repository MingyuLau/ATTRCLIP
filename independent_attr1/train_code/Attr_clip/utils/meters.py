...
import numpy as np
from fvcore.common.timer import Timer   # 这是一个计时器类，用于测量代码的执行时间
...
class ApMeter(object):
    ...
    def __init__(self, max_iter, cfg):
        ...
        self.mb_mAP = ScalarMeter(cfg.LOG_PERIOD)  # minibatch_map
        self.max_mAP = 0.0
    ...
    # def update_stats(self, preds, labels):
    #     """
    #     Update the current stats.
    #     """
    #     self.all_preds.append(preds)
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        self.all_preds = [t.cpu().detach().numpy() for t in self.all_preds]
        self.all_labels = [t.cpu().detach().numpy() for t in self.all_labels]
        self.mb_mAP = metrics.charades_map(self.all_preds, self.all_labels)
        self.max_mAP = max(self.max_mAP, self.mb_mAP)

        stats["mb_mAP"] = self.mb_mAP
