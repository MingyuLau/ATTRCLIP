...
def load_checkpoint(
    ...
    # xuyue: for CLIP
    if path_to_checkpoint.startswith("CLIP"):
        # for clip
        _, visual_ckpt, text_ckpt = path_to_checkpoint.split(":")
        load_checkpoint(
            visual_ckpt.strip(), ms.visual_net,
            False, optimizer, scaler, epoch_reset, squeeze_temporal)
        load_checkpoint(
            text_ckpt.strip(), ms.text_net,
            False, optimizer, scaler, epoch_reset, squeeze_temporal)
        return -1
    
    if path_to_checkpoint.startswith("Fuse"):
        # for clip
        _, frame_ckpt, video_ckpt = path_to_checkpoint.split(":")
        load_checkpoint(
            video_ckpt.strip(), ms.video_net,
            False, optimizer, scaler, epoch_reset, squeeze_temporal)
        load_checkpoint(
            frame_ckpt.strip(), ms.frame_net,
            False, optimizer, scaler, epoch_reset, squeeze_temporal)
        return -1
    ...
    elif "visual_net" in model_dict and "head" in checkpoint:
        pre_train_dict = {"visual_net": {}}
        for key, param in checkpoint.items():
            if key == "head":
                pre_train_dict["head_cls"] = param["head"]
            else:
                pre_train_dict["visual_net"][key] = param
    ...
    if not_load_layers:
        for k in not_load_layers:
            logger.info("Network weights {} ({}) not loaded.".format(k, model_dict[k].size()))
    # Weights that do not have match from the pre-trained model.
    not_use_layers = [
    ...
    if not_use_layers:
        for k in not_use_layers:
            logger.info("Network weights {} ({})not used.".format(k, pre_train_dict[k].size()))
