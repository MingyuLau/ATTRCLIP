...
from .Frozen_utils.base import BaseModel
from .Frozen_utils.video_transformer import SpaceTimeTransformer
from .Frozen_utils.util import state_dict_data_parallel_fix
    ...
    def __init__(self,
        ...
        if load_checkpoint not in ["", None]:
            ...
            if torch.cuda.device_count()>1:
                local_rank = dist.get_rank() # fix by xuyue
                checkpoint = torch.load(load_checkpoint, map_location='cuda:{}'.format(local_rank))
            else:
                checkpoint = torch.load(load_checkpoint, map_location='cuda:0')

            state_dict = checkpoint['state_dict']
            new_state_dict = state_dict_data_parallel_fix(state_dict, self.state_dict())
            new_state_dict = self._inflate_positional_embeds(new_state_dict)
            self.load_state_dict(new_state_dict, strict=True)
            print("Restored from checkpoint", load_checkpoint)
...
class FrozenWrapper(nn.Module):
    def __init__(self, cfg):
        ...
        self.Frozen_model = FrozenInTime(
            video_params={
            ...
            # load_checkpoint="./checkpoint/cc-webvid2m-4f_stformer_b_16_224.pth.tar",
            load_checkpoint="./checkpoint/egovlp.pth",
        )


    def __encode_image(self, image) -> torch.Tensor:
        return self.Frozen_model.compute_video(image)

    def __encode_text(self, text_token):
        return self.Frozen_model.compute_text({
            "input_ids": text_token,
            "attention_mask": torch.ones_like(text_token, dtype=int).to(text_token.device),
        })
